...
Write-Host "=== 99. POWERSHELLRICH ===" -Foreground Cyan

Write-Host "`n99.1. Проверка / установка модуля PowerShellRich" -Foreground Cyan

# 1. Проверяем, установлен ли модуль PowerShellRich
$moduleName = "PowerShellRich"
$installedModule = Get-InstalledModule -Name $moduleName -ErrorAction SilentlyContinue

if ($installedModule) {
    Write-Host "✅ Модуль '$moduleName' уже установлен." -ForegroundColor Green
    Write-Host "   Версия: $($installedModule.Version)" -ForegroundColor Cyan
} else {
    Write-Host "⚠️ Модуль '$moduleName' не найден. Начинаю установку..." -ForegroundColor Yellow
    
    # Проверка на наличие команды Install-Module (обычно есть в PowerShell 5.1+)
